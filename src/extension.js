                ...
                childProcess.exec(command, async (err, stdout, stderror) => {
                    ...
                    if (log && log.diagnostics) {
                        ...
                        log.diagnostics.forEach(d => {
                            ...
                            // Add error code with link to MQL5 documentation (errors vs warnings)
                            if (d.errorCode) {
                                const docPage = d.severity === vscode.DiagnosticSeverity.Error
                                    ? 'errorscompile'
                                    : 'warningscompile';
                                diag.code = {
                                    value: `MQL${d.errorCode}`,
                                    target: vscode.Uri.parse(`https://www.mql5.com/${getMql5DocLang()}/docs/constants/errorswarnings/${docPage}`)
                                };
                            }
                        ...
                        // Focus Problems panel after diagnostics are populated
...
function activate(context) {
    ...
    context.subscriptions.push(vscode.workspace.onDidChangeTextDocument((event) => {
        ...
        autoCheckTimer = setTimeout(async () => {
            ...
            try {
                await Compile(0); // Syntax check (no compilation)
            } finally {
    ...
    sleep(3000).then(async () => {
        ...
        if (editor) {
            ...
            if (['.mq4', '.mq5', '.mqh'].includes(ext)) {
                await Compile(0); // Syntax check on startup
            }
        }
    });
}
